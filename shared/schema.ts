...
export const sessionRegistrations = pgTable("session_registrations", {
  ...
  tier: text("tier").notNull(),              // "executive" | "leadership"
  amount: integer("amount").notNull(),        // 249 or 99
  sessionName: text("session_name").notNull().default("The Agentic Shift"),
  sessionDate: text("session_date").notNull().default("22 May 2026"),
  sessionVenue: text("session_venue").notNull().default("Virtual"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
