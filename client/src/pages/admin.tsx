...
import { ContactRecord, PartnershipRecord, CheckboxLead, SessionRegistration } from "@shared/schema";
import { Download, Users, MessageSquare, LogOut, Eye, EyeOff, ChevronUp, ChevronDown, Calendar, Filter, Clipboard, Ticket } from "lucide-react";
...
function AdminDashboard() {
  ...
  const [sessionRegSort, setSessionRegSort] = useState({ field: 'createdAt', direction: 'desc' as 'asc' | 'desc' });
  const [contactPage, setContactPage] = useState(1);
  const [partnershipPage, setPartnershipPage] = useState(1);
  const [checkboxLeadPage, setCheckboxLeadPage] = useState(1);
  const [sessionRegPage, setSessionRegPage] = useState(1);
  ...
  const { data: sessionRegs = [], isLoading: sessionRegsLoading } = useQuery<SessionRegistration[]>({
    queryKey: ["/api/admin/session-registrations"],
  });
  ...
  const filteredSessionRegs = filterByDate(sessionRegs);
  const sortedSessionRegs = sortData(filteredSessionRegs, sessionRegSort);
  const paginatedSessionRegs = paginateData(sortedSessionRegs, sessionRegPage);
  const totalSessionRegPages = Math.ceil(sortedSessionRegs.length / recordsPerPage);
  const sessionRevenue = sortedSessionRegs.reduce((sum, r) => sum + (r.amount || 0), 0);

  const handleSort = (field: string, type: 'contacts' | 'partnerships' | 'checkbox-leads' | 'session-registrations') => {
    if (type === 'contacts') {
      setContactSort(prev => ({
        field,
  ...
  const downloadCSV = async (type: 'contacts' | 'partnerships' | 'checkbox-leads' | 'session-registrations') => {
    try {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Session Registrations</CardTitle>
              <Ticket className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{sessionRegs.length}</div>
              <p className="text-xs text-muted-foreground">Revenue: ${sessionRegs.reduce((s, r) => s + (r.amount || 0), 0)}</p>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-4 items-center">
              ...
              <div className="flex items-center gap-4 text-sm text-gray-600">
                ...
                <span>Session Regs: {sortedSessionRegs.length} records</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Tabs defaultValue="contacts" className="w-full">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="partnerships">
              Partnerships ({sortedPartnerships.length})
            </TabsTrigger>
            <TabsTrigger value="demo-requests">
              Demo Requests ({sortedCheckboxLeads.length})
            </TabsTrigger>
            <TabsTrigger value="session-registrations">
              Session Regs ({sortedSessionRegs.length})
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="session-registrations">
            <Card>
              <CardHeader>
                <div className="flex justify-between items-center">
                  <CardTitle>
                    Agentic Shift Session Registrations
                    {sortedSessionRegs.length > 0 && (
                      <span className="ml-3 text-sm font-normal text-gray-500">
                        Revenue: ${sessionRevenue} USD
                      </span>
                    )}
                  </CardTitle>
                  <Button onClick={() => downloadCSV('session-registrations')} className="bg-green-700 hover:bg-green-800 text-white">
                    <Download className="mr-2 h-4 w-4" />
                    Export to Excel
                  </Button>
                </div>
              ...
              <CardContent>
                {sessionRegsLoading ? (
                  ...
                  <div className="text-center py-4 text-gray-500">No session registrations yet</div>
                ) : (
                  <div>
                    <div className="overflow-x-auto">
                      <Table>
                        <TableHeader>
                          <TableRow>
                            <TableHead>
                              <Button variant="ghost" onClick={() => handleSort('id', 'session-registrations')} className="h-auto p-0 font-semibold">
                                ID
                                {sessionRegSort.field === 'id' && (sessionRegSort.direction === 'desc' ? <ChevronDown className="ml-1 h-4 w-4" /> : <ChevronUp className="ml-1 h-4 w-4" />)}
                              </Button>
                            </TableHead>
                            <TableHead>
                              <Button variant="ghost" onClick={() => handleSort('name', 'session-registrations')} className="h-auto p-0 font-semibold">
                                Name
                                {sessionRegSort.field === 'name' && (sessionRegSort.direction === 'desc' ? <ChevronDown className="ml-1 h-4 w-4" /> : <ChevronUp className="ml-1 h-4 w-4" />)}
                              </Button>
                            </TableHead>
                            <TableHead>
                              <Button variant="ghost" onClick={() => handleSort('email', 'session-registrations')} className="h-auto p-0 font-semibold">
                                Email
                                {sessionRegSort.field === 'email' && (sessionRegSort.direction === 'desc' ? <ChevronDown className="ml-1 h-4 w-4" /> : <ChevronUp className="ml-1 h-4 w-4" />)}
                              </Button>
                            </TableHead>
                            <TableHead>Phone</TableHead>
                            ...
                            <TableHead>
                              <Button variant="ghost" onClick={() => handleSort('tier', 'session-registrations')} className="h-auto p-0 font-semibold">
                                Tier
                                {sessionRegSort.field === 'tier' && (sessionRegSort.direction === 'desc' ? <ChevronDown className="ml-1 h-4 w-4" /> : <ChevronUp className="ml-1 h-4 w-4" />)}
                              </Button>
                            </TableHead>
                            <TableHead>
                              <Button variant="ghost" onClick={() => handleSort('amount', 'session-registrations')} className="h-auto p-0 font-semibold">
                                Amount
                                {sessionRegSort.field === 'amount' && (sessionRegSort.direction === 'desc' ? <ChevronDown className="ml-1 h-4 w-4" /> : <ChevronUp className="ml-1 h-4 w-4" />)}
                              </Button>
                            </TableHead>
                            <TableHead>
                              <Button variant="ghost" onClick={() => handleSort('createdAt', 'session-registrations')} className="h-auto p-0 font-semibold">
                                Registered At
                                {sessionRegSort.field === 'createdAt' && (sessionRegSort.direction === 'desc' ? <ChevronDown className="ml-1 h-4 w-4" /> : <ChevronUp className="ml-1 h-4 w-4" />)}
                              </Button>
                            </TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {paginatedSessionRegs.map((reg) => (
                            <TableRow key={reg.id}>
                              ...
                              <TableCell className="font-medium text-xs">{(reg as any).sessionName || 'The Agentic Shift'}</TableCell>
                              <TableCell className="text-xs whitespace-nowrap">{(reg as any).sessionDate || '22 May 2026'}</TableCell>
                              <TableCell className="text-xs">{(reg as any).sessionVenue || 'Virtual'}</TableCell>
                              <TableCell>
                                <Badge variant={reg.tier === 'executive' ? 'default' : 'secondary'} className="capitalize">
                                  {reg.tier}
                                </Badge>
                              </TableCell>
                              <TableCell className="font-semibold">${reg.amount}</TableCell>
                              <TableCell className="text-xs whitespace-nowrap">
                                {reg.createdAt ? format(new Date(reg.createdAt), 'MMM d, yyyy HH:mm') : 'N/A'}
                              </TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                    ...
                    {totalSessionRegPages > 1 && (
                      <div className="flex items-center justify-between px-2 py-4">
                        <div className="text-sm text-gray-500">
                          Showing {((sessionRegPage - 1) * recordsPerPage) + 1} to{' '}
                          {Math.min(sessionRegPage * recordsPerPage, sortedSessionRegs.length)} of{' '}
                          {sortedSessionRegs.length} records
                        </div>
                        <div className="flex items-center space-x-2">
                          <Button variant="outline" size="sm" onClick={() => setSessionRegPage(prev => Math.max(prev - 1, 1))} disabled={sessionRegPage === 1}>
                            Previous
                          </Button>
                          <span className="text-sm">Page {sessionRegPage} of {totalSessionRegPages}</span>
                          <Button variant="outline" size="sm" onClick={() => setSessionRegPage(prev => Math.min(prev + 1, totalSessionRegPages))} disabled={sessionRegPage === totalSessionRegPages}>
                            Next
                          </Button>
                        </div>
                      </div>
