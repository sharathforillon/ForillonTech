...
if (!process.env.MAILJET_API_KEY || !process.env.MAILJET_SECRET_KEY) {
  console.warn("MAILJET_API_KEY or MAILJET_SECRET_KEY environment variable is not set");
} else {
  mailjet = new Mailjet({
...
export async function sendPartnershipInquiryViaMailjet(inquiry: PartnershipInquiry): Promise<boolean> {
  if (!mailjet) {
    console.error('Mailjet is not configured. Please set MAILJET_API_KEY and MAILJET_SECRET_KEY');
    return false;
  }
...
Reply directly to: ${inquiry.email}
    ...
    console.error('Mailjet error:', error.statusCode || error);
    return false;
  }
}
...
Reply directly to: ${lead.email}
    ...
    console.error('Mailjet error (admin notification):', error.statusCode || error);
    return false;
  }
}
...
Enterprise Survey & Research Solutions
    ...
    await request;
    console.log('✅ Lead confirmation email sent successfully via Mailjet');
    return true;

  } catch (error: any) {
    console.error('Mailjet error (lead confirmation):', error.statusCode || error);
    return false;
  }
}
